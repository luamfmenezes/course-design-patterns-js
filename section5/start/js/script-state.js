(function (win, $) {
  ...
  Circle.prototype.next = function (shape) {
    if (shape) {
  ...
  function eventDispatcherDecorator(object) {
    var list = {};
  ...
  o.addEventListener("over", function () {
    console.log("its over");
  });
  o.addEventListener("over", function () {
    console.log("its over2");
  });
  ...
  class RedState {
    constructor(object) {
      this.object = object;
      this._on = "red";
      this._off = "rgba(255,0,0,0.24)";
      this._nextState;
    }

    nextState(nextState) {
      this_nextState = nextState;
    }

    state() {
      this.object.color(this.on);
      setTimeout(() => {
        binder(_nextState, _nextState.start);
      }, 1000);
      setTimeout(() => {
        this.object.color(off);
      }, 3000);
    }
  }
